...
export function LabResultsTable({ onSampleEdit, onSampleApproval }: LabResultsTableProps) {
  ...
  const getGradeColor = (grade: string) => {
    switch (grade) {
      ...
      case "O'rta": return "text-yellow-700 bg-yellow-100";
  ...
  return (
    <Card className="border-none shadow-2xl rounded-[3rem] bg-white/40 backdrop-blur-3xl overflow-hidden animate-in fade-in duration-700">
      <CardHeader className="p-10 border-b border-white/20 bg-white/20">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              size="sm"
              onClick={() => {/* TODO: Export */ }}
              className="h-12 px-6 rounded-xl border-white/50 bg-white/50 hover:bg-white font-black text-[10px] uppercase tracking-widest shadow-sm transition-all active:scale-95"
            >
            ...
            <Button
              ...
              className="h-12 w-12 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-all active:scale-95"
            >
              <X size={20} />
            </Button>
          </div>
      ...
      <CardContent className="p-10 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-6">
          ...
          <div className="space-y-3">
            <Label className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] ml-2">Qidirish</Label>
            <div className="relative group">
              ...
              <Input
                ...
                className="pl-12 h-12 rounded-xl bg-white/50 border-white/50 focus:border-primary focus:ring-primary/5 transition-all font-bold text-sm"
              />
            </div>
          </div>
          ...
          {[
            ...
            <div key={i} className="space-y-3">
              <Label className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] ml-2">{filter.label}</Label>
              <div className="relative">
                <select
                  value={filter.value}
                  onChange={(e) => filter.setter(e.target.value as any)}
                  className="w-full h-12 pl-4 pr-10 rounded-xl bg-white/50 border-2 border-white/50 focus:border-primary focus:outline-none transition-all font-bold text-sm appearance-none cursor-pointer"
                >
        ...
        {/* Date Filters Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-secondary/30 rounded-[2rem] border border-white/50">
          <div className="space-y-3">
            <Label className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] ml-2">Sana (Dan)</Label>
            <Input
              type="date"
              value={dateFrom}
              onChange={(e) => setDateFrom(e.target.value)}
              className="h-12 rounded-xl bg-white/50 border-white/50"
            />
          </div>
          <div className="space-y-3">
            <Label className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] ml-2">Sana (Gacha)</Label>
            <Input
              type="date"
              value={dateTo}
        ...
        <div className="overflow-hidden rounded-[2rem] border border-white/50 bg-white/30 backdrop-blur-sm shadow-xl">
                    ...
                    >
                      <td className="p-6">
                        <div className="flex flex-col">
                          <span className="text-lg font-black text-foreground tabular-nums">#{toy?.orderNo || "???"}</span>
                          <span className="text-[10px] font-bold text-muted-foreground/60 uppercase">{Number(toy?.netto).toFixed(2)} KG</span>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-black text-foreground uppercase">{sample.markaLabel}</span>
                          <span className="text-[10px] font-bold text-primary uppercase italic">{sample.productType}</span>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-2 py-1 bg-secondary text-primary text-[10px] font-black rounded-lg border border-primary/10 uppercase">
                            NAV {sample.navi}
                          </span>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className="flex items-center justify-center gap-4 text-center">
                          <div className="flex flex-col min-w-[60px]">
                            <span className="text-[9px] font-black text-blue-500/50">NAM</span>
                            <span className="text-sm font-black tabular-nums">{sample.moisture}%</span>
                          </div>
                          <div className="flex flex-col min-w-[60px]">
                            <span className="text-[9px] font-black text-rose-500/50">IFL</span>
                            <span className="text-sm font-black tabular-nums">{sample.trash}%</span>
                          </div>
                          <div className="flex flex-col min-w-[60px]">
                            <span className="text-[9px] font-black text-emerald-500/50">PISH</span>
                            <span className="text-sm font-black tabular-nums">{sample.strength}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className={cn(
                          ...
                          sample.status === "PENDING" ? "bg-amber-500/10 text-amber-600 border border-amber-500/20" :
                            sample.status === "APPROVED" ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" :
                      ...
                      <td className="p-6">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-foreground tabular-nums">{formatDate(sample.createdAt)}</span>
                          <span className="text-[9px] font-bold text-muted-foreground/40 uppercase">GMT +5:00</span>
                        </div>
                      </td>
                      <td className="p-6">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            ...
                            className={cn(
                              ...
                              sample.showToSales ? "text-primary bg-primary/5" : "text-muted-foreground/30"
                            )}
                            disabled={sample.status !== "APPROVED"}
                          >
                            {sample.showToSales ? <Eye size={16} /> : <EyeOff size={16} />}
                          </Button>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onSampleEdit(sample)}
                            className="h-10 w-10 p-0 rounded-xl hover:bg-foreground hover:text-white transition-all"
                          >
                            <Edit size={16} />
                          </Button>
                          <Button
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-6 bg-white/20 rounded-[2rem] border border-white/20">
            <div className="text-[11px] font-black text-muted-foreground/60 uppercase tracking-widest">
              SHOWING <span className="text-foreground">{startIndex + 1} - {Math.min(startIndex + itemsPerPage, filteredSamples.length)}</span> OF {filteredSamples.length} DATA BLOCKS
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="px-6 py-2 bg-foreground text-white rounded-xl font-black tabular-nums text-sm shadow-lg">
        ...
        {filteredSamples.length === 0 && (
          <div className="text-center py-24 bg-white/10 rounded-[3rem] border-2 border-dashed border-white/20">
            <FlaskConical className="h-20 w-20 mx-auto mb-6 text-muted-foreground/20 animate-pulse" />
            <p className="text-xl font-black text-muted-foreground/40 uppercase tracking-widest">Tahlil natijalari topilmadi</p>
            <p className="text-[11px] font-bold text-muted-foreground/20 uppercase mt-2 tracking-[0.2em]">Filter parametrlarini o&apos;zgartirib ko&apos;ring</p>
          </div>
        )}
      </CardContent>
    </Card>
